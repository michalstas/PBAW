...
function validate(&$form,&$infos,&$msgs,&$hide_intro){
	...
	if ( $form['x'] == "") $msgs [] = 'Nie podano potrzebnej kwoty';
	if ( $form['y'] == "") $msgs [] = 'Nie podano dlugosci splaty';
	if ( $form['y'] == "") $msgs [] = 'Nie podano oprocentowania';
	...
	if ( count($msgs)==0 ) {
		...
		if (! is_numeric( $form['z'] )) $msgs [] = 'Druga wartość nie jest liczbą';
	}
...
// wykonaj obliczenia
function process(&$form,&$infos,&$msgs,&$result,&$rata){
	...
	//wykonanie operacji
	$result = $form['x'] + ($form['z'] * $form['y'])/100 * $form['x'];
	$rata = ((($form['z'] * $form['y'])/100 * $form['x'])/$form['y'])/12;
	$rata = round($rata, 2);
}
...
if ( validate($form,$infos,$messages,$hide_intro) ){
	process($form,$infos,$messages,$result,$rata);
}
